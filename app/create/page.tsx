...
export default function CreatePage() {
  ...
  function onPick(file: File | null) {
    if (!file) return;
    // 실제 변환 호출은 /api/convert + lib/convert.ts 추가 후 교체.
    setStage("converting");
    timerRef.current = window.setTimeout(() => setStage("result"), 1500);
  }
    ...
    <>
      ...
      {stage === "result" && (
        <section className="px-5 pt-4">
          ...
          <div className="mt-4 aspect-square w-full overflow-hidden rounded-2xl bg-white ring-1 ring-stone-200" aria-label="도안 미리보기">
            <SamplePreview />
          </div>
